...
if (io & IO_SDA) {
if (st == IO_LOW) GPIOB->BSRR |= (1 << (9+16));  // no BSRR in new 
//GPIO_typedef, so manually shift by another 16 - FOSTER
else              GPIOB->BSRR |= (1 << 9);
}
if (io & IO_SCL) {
...
*/
I2C1->CR2    =  0x001E;               /* Clock: 30MHz, interrupts 
disabled  */
...
I2C1->CR1   |=  I2C_CR1_PE;           /* Enable peripheral                  
*/
I2C1->CR1   |= 0x0400;                /* I2C mode, General call 
disabled,   */
...
*/
I2C1->CR1 |= I2C_CR1_SWRST;           /* Put I2C peripheral in reset 
mode   */
...
*/
for (i = 0; i < 9; i++) {
if (io_state (IO_SDA) == IO_HI) break;  /* Break if SDA released by 
slave */
...
I2C1->DR = byte;
return (WaitFlagSet (I2C_SR1_TXE |I2C_SR1_BTF));   /* Byte transfer 
finished bit is set */
}
/*------------------------------------------------------------------------
-----
...
*------------------------------------------------------------------------
----*/
uint32_t I2C_WrData (uint8_t addr, uint8_t secByte, uint8_t *buf, uint32_t 
cnt) {
uint8_t  *dp  = buf;
uint32_t  num = cnt;
uint32_t  err = 0;
...
*------------------------------------------------------------------------
----*/
uint32_t I2C_RdData (uint8_t addr, uint8_t secByte, uint8_t *buf, uint32_t 
cnt) {
uint8_t  *dp  = buf;
uint32_t  num = cnt;
uint32_t  err = 0;
